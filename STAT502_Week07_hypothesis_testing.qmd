...
pairing with each other (we would compare `smith` _before_ with
`smith` _after_).
...
First of all: "Interval" is another word for "range".  The basic idea is that we
estimate a *range* of values, and we hope that the true value of our
parameter is within this range.  We ensure that the probability of
this range including the true value of the parameter is 95\% (other
values such as 99\% or 99.5\% are sometimes used, but we will stick to
95\% here).
...
for(i in 1:100){
    ...
    segments(x0=jj[1],y0=i/10,x1=jj[2],y1=i/10,col=col,lwd=1)
}
abline(v=0)    
```
...
ANOVA ("Analysis of variance") is a powerful technique that allows one
to test whether a continuous measurement dependes on a _discrete_
variable.  Consider the following dataset:


```{r label=plantgrowthexample}
PlantGrowth
```

This shows the plant mass for a crop under a control and two different
treatments, labeled `ctrl`, `trt1` and `trt2` respectively (to get
more doucmentation on this dataset, type `?PlantGrowth` at the R
command line).  Of course, the question we are going to answer is
whether the treatments differ from the control.

The first step is a graphical analysis

```{r label=plantgrowthplot} plot(weight~group,data=PlantGrowth) ```
[above, one would read `weight~group` as "weight _as a function of_
group"; the "`~`" symbol (which appears on the top left of most
keyboards) means "is a function of".  The tilde notation is a powerful
tool and we do not have time to go in to all its functionality.  For
further details, consult the help page at `?formula`].  We see that it
is not immediately clear whether or not the treatments are effective.
To answer this, we consider a null hypothesis $H_0$ which is that the
treatment has no effect on the growth of the plants.  We quantify the
evidence against this null by calculating a $p$-value, and this is
accomplished easily in R:


```{r label=plantgrowthanova}
lm(weight~group,data=PlantGrowth)
summary(lm(weight~group,data=PlantGrowth))
```


The summary is a little harder to interpret than the output of
`t.test()`.  Firstly, R puts the treatments in alphabetical order, and
the first one is the control group, being `ctrl`.  We see that the
mean growth for the control group is `5.0320`, from the `Estimate`
column.  The $p$-value at the right hand side, reading `<2e-16`, is
against a null that the control group has zero growth.

The next two lines are relative to the control group.  The second
line, `grouptrt1`, refers to treatment group 1, `trt1`.  The estimate
of `-0.3710` refers to the growth _relative to the control group_.
This would be an estimate of $5.0320-0.3710=4.661$.  The $p$-value of
0.1944 is against a null that treatment 1 has no effect on growth.
The third line similarly shows the effect of treatment 2, again
relative to the control group.  We see that the additional growth for
`trt2` is estimated as `0.4940`, and the $p$- value is 0.0877.

Pn the final line we see a $p$-value of 0.01591 which corresponds to a
null that all three treatments have the same mean plant growth.  We
reject this hypothesis at 5\%.
